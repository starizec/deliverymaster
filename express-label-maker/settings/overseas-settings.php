...
function explm_overseas_tab_content() {
    ...
    if ( isset($_POST['explm_overseas_nonce']) && wp_verify_nonce( sanitize_text_field( wp_unslash($_POST['explm_overseas_nonce']) ), 'explm_save_overseas_settings') ) {

        // DELETE – remove ALL related options (Overseas + Sender)
        if ( isset($_POST['delete_overseas_account']) ) {
            $delete_keys = [
                // Overseas settings
                'explm_overseas_api_key_option',
                'explm_overseas_enable_pickup',
                'explm_overseas_pickup_shipping_method',
                'explm_overseas_customer_note',
                // Sender info
                'explm_overseas_company_or_personal_name',
                'explm_overseas_contact_person',
                'explm_overseas_street',
                'explm_overseas_property_number',
                'explm_overseas_city',
                'explm_overseas_postal_code',
                'explm_overseas_phone',
                'explm_overseas_email',
                'explm_overseas_country',
            ];
            ...
            // Sender info (right column) – ALL REQUIRED
            ...
            // Backend validation: API key + ALL sender fields required
            if ( empty($api_key) ) {
                ...
                if (!$missing) {
                    ...
                    // Update lockers if enabled and method present
                    if ( '1' === $enable_paketomat && !empty($shipping_method) && class_exists('ExplmParcelLockers') ) {
                        (new ExplmParcelLockers())->explm_update_overseas_parcelshops_cron_callback();
                    }
    ...
    // LEFT: Overseas settings
    echo '<div style="flex:1 1 420px;min-width:320px">';
    echo '<h3>' . esc_html__('Overseas Settings', 'express-label-maker') . '</h3>';
    echo '<table class="form-table">';

    echo '<tr><th scope="row"><label for="explm_overseas_api_key">' . esc_html__('API Key', 'express-label-maker') . ' ';
    echo '<span style="cursor:help;" title="' . esc_attr__('Request your API key by emailing cs@overseas.hr.', 'express-label-maker') . '">ℹ️</span>';
    echo '</label></th>';
    echo '<td><input name="explm_overseas_api_key" type="text" id="explm_overseas_api_key" value="' . esc_attr($saved_api_key) . '" class="regular-text" required></td></tr>';
    ...
    echo '<td><select name="explm_overseas_shipping_method" id="explm_overseas_shipping_method">';
    ...
    echo '<tr><th scope="row"><label for="explm_overseas_customer_note">' . esc_html__('Customer Note', 'express-label-maker') . ' ';
    ...
    echo '</label></th>';
    echo '<td><textarea name="explm_overseas_customer_note" id="explm_overseas_customer_note" rows="3" cols="40" maxlength="35">' . esc_textarea($customer_note) . '</textarea></td></tr>';
    ...
    echo '<p class="submit">';
    echo '<input type="submit" name="submit" class="button button-primary" value="' . esc_attr__('Save Changes', 'express-label-maker') . '">';
    echo '<input type="submit" name="delete_overseas_account" class="button" value="' . esc_attr__('Delete Account', 'express-label-maker') . '" style="background-color:transparent;color:red;border:1px solid red;margin-left:10px;">';
    echo '</p>';
